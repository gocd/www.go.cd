...
namespace :static_checks do
  ...
  options = {
      ...
      :ignore_urls          => [/http(s?):\/\/(www\.)?(localhost|github|blogs\.oracle|etsy|.*openjdk|.*\.stackexchange|askubuntu|gartner|linkedin|twitter|www.netspi|.*hashnode|papers\.ssrn|wired|forbes|medium|hub\.docker|kubernetes\.io|gauge\.org|noris\.de|alpinelinux\.org|puppetlabs\.com)/],
